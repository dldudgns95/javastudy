...
public class MainWrapper {
	...
	public static void q7() {
		...
		System.out.println("배열에 포함된 " + TARGET + "는 " + count + "개 입니다.");
	}
	...
	public static void q8() {
		...
		if(Integer.parseInt(strScore) >= 60) {
		  strResult += "행주";
		}
		if(Integer.parseInt(strScore) >= 70) {
		  strResult += "도마";
		}
		if(Integer.parseInt(strScore) >= 80) {
		  strResult += "식칼";
		}
	...
	public static void q9() {
		...
		for(int i = 1; i <= 100; i++) {
		  if((i % 10) % 3 == 0 && (i % 10) != 0) {       //일의자리 배수
		    condition1 = true;
		  } else {
		    condition1 = false;
		  }
		  if((i / 10) % 3 == 0 && i >= 10) {             // 십의 자리 배수
		    condition2 = true;
		  } else {
		    condition2 = false;
		  }
		  
		  if(condition1 && condition2) {            // 둘다 true 이면 "짝짝" 출력
		    System.out.print(String.format("%-3s", "짝짝"));
		  } else if(condition1 || condition2) {     // 둘중 하나라도 true이면 "짝" 출력
		    System.out.print(String.format("%-4s", "짝"));
		  } else {                                                  // 그 외에는 숫자 출력
		    System.out.print(String.format("%-5d", i));
		  }
		  
		  if((i % 10) == 0) System.out.println();       // 10의 배수 줄바꿈
		}
	}
