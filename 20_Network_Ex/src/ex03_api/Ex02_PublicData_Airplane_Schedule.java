...
public class Ex02_PublicData_Airplane_Schedule {
  ...
  /*
   * 한국공항공사_항공기 운항정보 - 국제선 공항 스케쥴
   * 1. 요청주소 : http://openapi.airport.co.kr/service/rest/FlightScheduleList/getDflightScheduleList
   * 2. 요청변수(Request Parameter)
   *    1) serviceKey      : 필수, 인코딩된 인증키
   *    2) pageNO          : 필수, 1 (페이지 번호)
   *    3) schDate         : 선택, 검색일자(20151005)
   *    4) schDeptCityCode : 선택, 출발도시코드(GMP)
   *    5) schArrvCityCode : 선택, 도착도시코드(HND)
   * postman 사이트에서 testing 가능
   */
  ...
  public static void ex03() {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(file))){
      ...
      for(int i = 0; i < items.length(); i++) {
        ...
        // System.out.println(item.getString("airlineKorean"));
        // System.out.println(item.getString("airlineEnglish"));
