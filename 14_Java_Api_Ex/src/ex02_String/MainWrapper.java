...
public class MainWrapper {
	...
	public static void ex04() {
		...
		int born = 0;
		int maleCheck = Integer.parseInt(personalId.substring(personalId.indexOf("-") + 1,personalId.indexOf("-") + 2));
		String male;
		if(maleCheck >= 3) {
			born += 2000 + Integer.parseInt(personalId.substring(0, 2));
		} else {
			born += 1900 + Integer.parseInt(personalId.substring(0, 2));
		}
		int age = calendar.get(Calendar.YEAR) - born;

		if(maleCheck == 1 || maleCheck == 3) {
			male = "남자";
		} else {
			male = "여자";
		}

		System.out.println(age + "살 " + male + "입니다.");
	}
	...
	public static void ex05() {
		...
		if(str.equals(reverse)) {
			System.out.println(str + " : 거꾸로 읽어도 " + reverse + "입니다.");
		} else {
