...
public class MainWrapper {
  ...
  public static void ex02() {
    ...
    try {
      ...
      if(!(dir.exists())) {
        dir.mkdirs();
        System.out.println("C:/storage 파일 생성 완료");
      }
      ...
      if(file.exists()) {
        file.delete();
        System.out.println(file + "파일 삭제 완료");
      } else {
        file.createNewFile();   // 반드시 예외 처리를 해야 하는 코드(Checked Exception인 IOException)
  ...
  public static void ex03() {
    ...
    //디렉터리에 있는 모든 파일 객체(파일, 디렉터리) 가져오기
    ...
    for(int i = 0; i < files.length; i++) {
      ...
      // File 객체 최종수정일
      long lasfModified = file.lastModified();
      String strLastModified = new SimpleDateFormat("yyyy-MM-dd a h:mm").format(lasfModified);
